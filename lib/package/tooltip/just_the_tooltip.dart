...
import 'package:nucleus_ui/package/tooltip/models/target_information.dart';
import 'package:nucleus_ui/package/tooltip/models/tooltip_controller.dart';
import 'package:nucleus_ui/package/tooltip/models/tooltop_interface.dart';
import 'package:nucleus_ui/package/tooltip/positioned_tooltip.dart';
import 'package:nucleus_ui/package/tooltip/tooltip_area.dart'
    show InheritedTooltipArea, TooltipArea;

part 'tooltip_entry.dart';
...
class JustTheTooltip extends StatefulWidget implements HoverInterface {
  const JustTheTooltip({
    ...
    this.onShow,
    this.controller,
    // With the new [triggerMode] field isModal's only function is to keep
  ...
  // This happens in the non-hover (i.e. isModal) case as well.
  @override
  ...
  @override
  TooltipState<OverlayEntry> createState() => _TooltipOverlayState();
}

class _TooltipOverlayState extends TooltipState<OverlayEntry> {
  @override
  OverlayEntry? entry;
    ...
    setState(
      () {
        ...
        if (widget.isModal) {
          ...
          overlay
            ..insert(skrimOverlay)
            ..insert(entryOverlay, above: skrimOverlay);
        } else {
          entry = entryOverlay;
...
// This looks more idiomatic as a mixin.
abstract class TooltipState<T> extends State<HoverInterface>
  ...
  void _attachController(TooltipController controller) {
    controller.attach(
  ...
  @override
  void didUpdateWidget(covariant HoverInterface oldWidget) {
  ...
  Future<void> _handlePress() async {
    ...
    if (tooltipCreated && enableFeedback) {
      if (triggerMode == TooltipTriggerMode.longPress && mounted) {
        ...
        if (mounted) await Feedback.forTap(context);
      }
    }
  }
  ...
  TargetInformation _getTargetInformation(BuildContext context) {
    ...
    // Instead of this, change the alignment on
